(function syncResearchPageMeta() {
    function run() {
        if (!window.i18n || !document.querySelector('.research-page')) {
            return;
        }
        var t = window.i18n.t;
        var pt = t('researchPage.pageTitle');
        if (pt && pt !== 'researchPage.pageTitle') {
            document.title = pt;
        }
        var pd = t('researchPage.pageDescription');
        if (pd && pd !== 'researchPage.pageDescription') {
