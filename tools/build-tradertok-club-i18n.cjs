/**
 * Merges tools/tradertok-club-flat/<lang>.json into locales/<lang>.json under tradertokClubPage.
 * Run: node tools/build-tradertok-club-i18n.cjs
 */
"use strict";
...
const flatDir = path.join(__dirname, "tradertok-club-flat");
...
for (const [lang, fname] of Object.entries(LANG_FILES)) {
  ...
  j.tradertokClubPage = nestFromFlat(flat);
  fs.writeFileSync(localePath, JSON.stringify(j, null, 2) + "\n", "utf8");
  console.log("updated tradertokClubPage in", fname);
}
