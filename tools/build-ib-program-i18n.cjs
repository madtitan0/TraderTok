/**
 * Merges tools/ib-program-flat/<lang>.json into locales/<lang>.json under ibProgramPage.
 * Run: node tools/build-ib-program-i18n.cjs
 */
"use strict";
...
const flatDir = path.join(__dirname, "ib-program-flat");
...
for (const [lang, fname] of Object.entries(LANG_FILES)) {
  ...
  j.ibProgramPage = nestFromFlat(flat);
  fs.writeFileSync(localePath, JSON.stringify(j, null, 2) + "\n", "utf8");
  console.log("updated ibProgramPage in", fname);
}
