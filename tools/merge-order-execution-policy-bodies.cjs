"use strict";
/**
 * Merges tools/i18n-overrides/bundles/orderExecutionPolicy.bodies.<locale>.json
 * into locales/<locale>.json under orderExecutionPolicyPage.
 *
 * Usage: node tools/merge-order-execution-policy-bodies.cjs en | all | vn ...
 */
const fs = require("fs");
...
function mergeLocale(locale) {
  ...
  loc.orderExecutionPolicyPage = Object.assign(
    {},
    loc.orderExecutionPolicyPage || {},
    bodies
  );
...
const targets =
  !args.length || args.includes("all")
    ? ALL.filter((locale) =>
        fs.existsSync(
          path.join(bundlesDir, `orderExecutionPolicy.bodies.${locale}.json`)
        )
      )
    : args;
...
if (!targets.length) {
  console.error(
    "Usage: node tools/merge-order-execution-policy-bodies.cjs <locale> [...] | all"
  );
  process.exit(1);
}
