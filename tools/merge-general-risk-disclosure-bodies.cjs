"use strict";
/**
 * Merges tools/i18n-overrides/bundles/generalRiskDisclosure.bodies.<locale>.json
 * into locales/<locale>.json under generalRiskDisclosurePage.
 *
 * Usage: node tools/merge-general-risk-disclosure-bodies.cjs en | all | vn ...
 */
const fs = require("fs");
...
function mergeLocale(locale) {
  ...
  loc.generalRiskDisclosurePage = Object.assign(
    {},
    loc.generalRiskDisclosurePage || {},
    bodies
  );
...
const targets =
  !args.length || args.includes("all")
    ? ALL.filter((locale) =>
        fs.existsSync(
          path.join(bundlesDir, `generalRiskDisclosure.bodies.${locale}.json`)
        )
      )
    : args;
...
if (!targets.length) {
  console.error(
    "Usage: node tools/merge-general-risk-disclosure-bodies.cjs <locale> [...] | all"
  );
  process.exit(1);
}
